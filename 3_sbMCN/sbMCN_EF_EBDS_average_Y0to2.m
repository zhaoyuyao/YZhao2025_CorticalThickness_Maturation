...
outputFolder = '.../Outputs_02/reg_gender_ROIave_no_MVM_thresholdZ2_WB_AAL_Y0to2';
figureFolder = '.../Figures_02/reg_gender_ROIave_no_MVM_thresholdZ2_WB_AAL_Y0to2';
...
fileToRead = fullfile(folderPath, 'sbMCN_avg_z_sort_WB_no_MVM_Y0to2.csv');
...
% Filter subjects who have at least two time points,
% with the first time point at 0 or 1 years and the final time point at 1 or 2 years
validSubjects = uniqueSubjects(cellfun(@(x) ...
    numel(x) >= 2 && ismember(x(1), minStartPoints) && ismember(x(end), maxEndPoints), subjectTimePoints));
